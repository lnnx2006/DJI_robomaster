...
// Get potential bounding boxes for all test images
void Objectness::getObjBndBoxesForTestsFast(CMat &image, vector<vector<Vec4i> > &_boxesTests, int numDetPerSize)
{
	//setColorSpace(HSV);
    //trainObjectness(numDetPerSize);
//    loadTrainedModel();
...
#pragma omp parallel for
    for (int i = 0; i < TestNum; i++)
    {
        // Here is reading of Test Images
//        imgs3u[i] = imread(format(_S(_voc.imgPathW), _S(_voc.testSet[i])));
        boxesTests[i].reserve(10000);
    }
...
#pragma omp parallel for
        for (int i = 0; i < TestNum; i++)
        {
            printf("SBSBSBSBSBBSBSBSBSBSBBSBS 11111!!!\n");
//          getObjBndBoxes(imgs3u[i], boxesTests[i], numDetPerSize);
            getObjBndBoxes(image, boxesTests[i], numDetPerSize);

            printf("SBSBSBSBSBBSBSBSBSBSBBSBS 3333!!!\n");
        }
...
#pragma omp parallel for
    ...
    {
//        CStr fName = _bbResDir + _voc.testSet[i];
        CStr fName = format("/home/exbot/Desktop/VOCdevkit/VOC2007/Results/BBoxesB2W8MAXBGR/3001");
        ValStructVec<float, Vec4i> &boxes = boxesTests[i];
        FILE *f = fopen(_S(fName + ".txt"), "w");
        fprintf(f, "%d\n", boxes.size());
        for (size_t k = 0; k < boxes.size(); k++)
